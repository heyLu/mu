...
var (
	...
	d1Retract = index.NewDatom(0, 1, "hey", 2, false)
...
func TestAddDatoms(t *testing.T) {
	eavt := New(index.CompareEavt)
	tu.ExpectEqual(t, eavt.Count(), 0)

	eavt = eavt.AddDatoms([]index.Datom{d1, d2, d3})
	tu.ExpectEqual(t, eavt.Count(), 3)

	eavt = eavt.AddDatoms([]index.Datom{d1Retract})
	tu.ExpectEqual(t, eavt.Count(), 2)
}
